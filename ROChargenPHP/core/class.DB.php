...
final class DB
{
	...
	static public function get_head_pal_path($id,$sex,$pal,$job_id)
	{
		$sex = self::$ascii_sex[$sex];
		if( $job_id == 4217 || $job_id == 4218 || $job_id == 4219 || $job_id == 4220 ) {
			return $pal ? "data/palette/도람족/머리/머리{$id}_{$sex}_{$pal}.pal" : false;
		}
		else {
	...
	static public function get_weapon_path($job_id, $sex, $weapon_id )
	{
		...
		return isset(self::$clothes[$job_id]) ? "data/sprite/방패/". self::$clothes[$job_id] ."/". self::$clothes[$job_id] ."_{$sex}_{$weapon_id}" : false;
	}
	...
	static public function get_shield_path($job_id, $sex, $shield_id )
	{
		...
		return isset(self::$clothes[$job_id]) ? "data/sprite/방패/". self::$clothes[$job_id] ."/". self::$clothes[$job_id] ."_{$sex}_{$shield_id}" : false;
	}
	...
	static public function get_robe_path( $job_id, $sex, $robe_id )
	{
		...
		return "data/sprite/로브/". self::$robes['list'][$robe_id]['name'] ."/{$sex}/". self::$clothes[$job_id] ."_{$sex}";
	}
