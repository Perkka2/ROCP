...
last_x, last_y, save_map, save_x, save_y, online, lpad(bin(`option`), 14, 0)
...
WHERE account_id = %d
");
DEFINE('GET_GMLEVEL', "SELECT `level`
FROM login
WHERE account_id = %d
");
DEFINE('IS_ONLINE', "SELECT * FROM `char` WHERE online = 1 AND account_id = %d");
...
DEFINE('GET_ONLINE', "SELECT count(*) FROM `char` WHERE online = 1 GROUP BY online");
...
// item_search.php
DEFINE('LIST_ITEMS', "SELECT ID, name_japanese FROM $cp.item_db WHERE name_english <> 'Unknown_Item' ORDER BY name_japanese");
...
// mvp_ladder.php
DEFINE('GET_MVP_DATE', "SELECT mvp_date FROM $logs.mvplog ORDER BY mvp_id");
...
// view_db.php
DEFINE('SHOW_FULL_ITEMS', "SELECT id, name_japanese, type, price_buy, weight, attack, defence,
...
DEFINE('COUNT_FULL_MOBS', "SELECT $cp.mob_db.Name2
FROM $cp.mob_db
%s AND exp > 0
ORDER BY $cp.mob_db.Name2
");
DEFINE('SHOW_FULL_MOBS', "SELECT $cp.mob_db.Name2, LV, HP, EXP, JEXP, ATK1, ATK2, mob_db.DEF, MDEF,
...
DEFINE('SHOW_ONLINE', "SELECT char.account_id, char.name, class, base_level, job_level, %s last_map
FROM `char`
LEFT JOIN $cp.privilege ON char.account_id = $cp.privilege.account_id
LEFT JOIN $cp.whosonline_ignore ON char.account_id = $cp.whosonline_ignore.account_id
WHERE $cp.whosonline_ignore.account_id IS NULL AND online = 1
%s
%s
...
DEFINE('SHOW_OIGNORED', "SELECT $cp.whosonline_ignore.account_id, login.userid
FROM $cp.whosonline_ignore, login
WHERE $cp.whosonline_ignore.account_id = login.account_id
ORDER BY account_id
");
DEFINE('ADD_OIGNORED', "INSERT INTO $cp.whosonline_ignore VALUES (%d)");
DEFINE('DEL_OIGNORED', "DELETE FROM $cp.whosonline_ignore WHERE account_id = %d");
